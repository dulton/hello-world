...
struct snkNode_liveHTTP {
	struct ammo_snk_node base;
	void *cgi;
	struct ammo_frame *mjframe;
	u8_t *frame_ptr;               // the pointer for multiple file-reads
	int mjframe_stale_cnt;         // preventing the sending of HTTP-MJPEG, thus stop moving forward the frames 
	int mjframe_len;
	int mode;
	int refresh;
};
...
#define cast_HttpNode(n)    ((struct snkNode_liveHTTP *)(n))
#define cast_RtpMedia(p)    ((struct rtp_media *)((p)->data))   /* p: points to struct ammo_pipe */
#define cast_UdpMedia(p)    ((struct udp_media *)((p)->data))   /* p: points to struct ammo_pipe */
#define cast_UdpNode(n)    ((struct snkNode_liveUDP *)(n))
//*******************************************************************************************************
#ifdef __AIT_DEPRECATE_OLD_PROTO_CODES__
struct req {
	struct conn *conn;
