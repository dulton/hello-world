...
void WLAN_SYS_StatusCallback(wlan_event_msg_t* event)
{
	...
	case WLAN_E_LINK:
		...
		if(event->flags){
			if( Netapp_get_cur_wifi_mode() == NETAPP_WIFI_STATION_MODE &&
	    		...
	    		Netapp_wifi_link_report(1);
			}
		}
		else{
			//link down 
			Netapp_wifi_link_report(0);
		}
	    break;
...
static UINT32 WLAN_SYS_Init9619(void)
{
    ...
    AHC_ConfigGPIOPad(NCFG_WAKEUP_PIN, PAD_NORMAL_TRIG);
    AHC_ConfigGPIO( NCFG_WAKEUP_PIN, AHC_FALSE );

    // WIFI module's RESET output, active-low
    // printc("Config WIFI GPIO - RESET\r\n");
    AHC_ConfigGPIOPad(NCFG_RESET_PIN, PAD_OUT_DRIVING(0));
    AHC_ConfigGPIO( NCFG_RESET_PIN,  AHC_TRUE  );
    AHC_SetGPIO( NCFG_RESET_PIN, 1 );
...
void WLAN_STA_CalculateChannel(UINT8* pubChannelList)
{
	...
	if (list->count > 0) {
        ...
        for (i = 0; i < list->count; i++, bi = (WLAN_BSS_INFO*)((BYTE*)bi + bi->length)) {
            ...
            UINT32 uiChannel = WLAN_CHSPEC_CHANNEL(bi->chanspec);
            UINT32 i, uiDelta;
            
            //printc("Current Channel : %d \n", WLAN_CHSPEC_CHANNEL(bi->chanspec));
            ...
            for(i = uiChannel; i < (uiChannel + uiDelta + 1); i++){
                pubChannelList[i]++;
...
void WLAN_STA_AutoSelectChannel_PanaRule(UINT32* puiChannel)
{
    ...
    UINT32 uiScanBufLength = WLAN_SCAN_BUF_SIZE;
	WLAN_SCAN_RESULT *list;
    UINT8  ubChannelCntList[WLAN_CHANNEL_13 + 1];
    UINT32 uiMinCnt = 0;
    
    if(m_bWiFiStatus == AHC_FALSE)
        return;
        
    memset(ubChannelCntList,0,sizeof(ubChannelCntList));
	...
	if (list->count > 0) {
        ...
        for (i = 0; i < list->count; i++, bi = (WLAN_BSS_INFO*)((BYTE*)bi + bi->length)) {
            //printc("==Current Channel : %d \n", WLAN_CHSPEC_CHANNEL(bi->chanspec));
            ubChannelCntList[WLAN_CHSPEC_CHANNEL(bi->chanspec)]++;
        }
	}

	mem_free(pScanMemPtr);
	
    uiMinCnt = ubChannelCntList[WLAN_CHANNEL_1];
    i = WLAN_CHANNEL_3;
    
    do{
    	if(uiMinCnt > ubChannelCntList[i])
    		uiMinCnt = ubChannelCntList[i];
    	...
    	switch(i)
    	{
    		...
    		case WLAN_CHANNEL_8:
    			i = WLAN_CHANNEL_11;
    		break;
    		case WLAN_CHANNEL_11:
    			i = WLAN_CHANNEL_12;
    		break;
    	}
    	
    }while(i < WLAN_CHANNEL_12);
    
    if(ubChannelCntList[WLAN_CHANNEL_6] == uiMinCnt)
    	*puiChannel = 6;
    else if(ubChannelCntList[WLAN_CHANNEL_1] == uiMinCnt)
    	*puiChannel = 1;
    else if(ubChannelCntList[WLAN_CHANNEL_3] == uiMinCnt)
    	*puiChannel = 3;
    else if(ubChannelCntList[WLAN_CHANNEL_8] == uiMinCnt)
    	*puiChannel = 8;
    else
    	*puiChannel = 11;
