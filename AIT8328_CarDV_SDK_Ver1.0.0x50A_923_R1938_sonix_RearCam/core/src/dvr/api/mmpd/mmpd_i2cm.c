...
MMP_ERR MMPD_I2cm_WriteBurstData(MMP_I2CM_ATTR *attr, MMP_USHORT usSubAdd, MMP_USHORT usData, MMP_ULONG ulDataAddr)
{
	...
	status |= MMPF_I2cm_WriteBurstData(attr, usSubAdd, (MMP_USHORT *)ulDataAddr, usData);
	return status;
}
...
//*----------------------------------------------------------------------------
// function : MMPD_I2cm_SetRxTimeout
// input    : 
// output   : none
// descript : ulTimeOut = 0 ==> no Timeout, otherwise the unit is millisecond
//*----------------------------------------------------------------------------
/**
 @brief Set HW I2CM read timeout

 Parameters:
 @param[in] p_i2cm_attribute : I2CM attribute, please refer the structure MMP_I2CM_ATTR
 @param[in] ulTimeOut The timeout value.
 @return the status of the operation.
*/
MMP_ERR MMPD_I2cm_SetRxTimeout(MMP_I2CM_ATTR *attr, MMP_ULONG ulTimeOut)
{
	MMP_ERR status;

	status |= MMPF_I2cm_SetRxTimeout(attr, ulTimeOut);
	return status;
}
