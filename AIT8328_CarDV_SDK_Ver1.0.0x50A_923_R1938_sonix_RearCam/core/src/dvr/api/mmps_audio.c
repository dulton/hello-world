...
static MMPS_AUDIO_I2S_CFG   m_AudioI2SConfig[I2S_CH_NUM];
...
static  MMPS_AUDIO_MP3_INFO		*m_Mp3Info;
static  MMPS_AUDIO_AACPLUS_INFO	*m_AacplusInfo;
static  MMPS_AUDIO_OGG_INFO		*m_OggInfo;
static  MMPS_AUDIO_RA_INFO		*m_RaInfo;
static  MMPS_AUDIO_WMA_INFO		*m_WMAproInfo;
static  MMPS_AUDIO_FLAC_INFO	*m_FLACInfo;
static  MMPS_AUDIO_WAV_INFO		*m_WavInfo;
static  MMPS_AUDIO_MIDI_INFO    *m_MIDIInfo;
static  MMPS_AUDIO_AMR_INFO     *m_AmrInfo;
...
/**
    ...
    @param[in] bFastCharge enable faster DAC turn on speed
    @return MMP_ERR_NONE, MMP_SYSTEM_ERR_CMDTIMEOUT

    @pre Before change setting, DAC should not power on yet. Otherwise,
         the setting will be applied until next time DAC power on.
*/
...
//------------------------------------------------------------------------------
//  Function    : MMPS_AUDIO_SetBypassPass
//  Description :
//------------------------------------------------------------------------------
/**
    @brief  Set Audio Bypass Path
    @param[in] bypasspath switch bypass pin
        - MMPS_AUDIO_BYPASS_DISABLE
        - MMPS_AUDIO_BYPASS_LL:     bypass L-in to L-out
        - MMPS_AUDIO_BYPASS_LR:     bypass L-in to R-out
        - MMPS_AUDIO_BYPASS_RL:     bypass R-in to L-out
        - MMPS_AUDIO_BYPASS_RR:     bypass R-in to R-out
    @return MMP_ERR_NONE, MMP_SYSTEM_ERR_CMDTIMEOUT
*/
MMP_ERR MMPS_AUDIO_SetBypassPath(MMPS_AUDIO_BYPASS bypasspath)
{
    return MMPD_AUDIO_SetBypassPath(bypasspath);
}
#if 0
...
MMP_ERR MMPS_AUDIO_InitializeI2SConfig(MMP_UBYTE ch, MMPS_AUDIO_I2S_CFG *I2SConfig)
{
    ...
    if ((I2SConfig->workingMode > MMPS_AUDIO_I2S_WORK_MODE_NONE) &&
        (I2SConfig->workingMode < MMPS_AUDIO_I2S_MAX_WORK_MODE)) {
        m_AudioI2SConfig[ch].workingMode = I2SConfig->workingMode;
    }
    else {
        err = MMP_AUDIO_ERR_PARAMETER;
        m_AudioI2SConfig[ch].workingMode = MMPS_AUDIO_I2S_MASTER_MODE;
    }
    if ((I2SConfig->mclkMode > MMPS_AUDIO_I2S_MCLK_MODE_NONE) &&
        (I2SConfig->mclkMode < MMPS_AUDIO_I2S_MAX_MCLK_MODE)) {
        m_AudioI2SConfig[ch].mclkMode = I2SConfig->mclkMode;
    }
    else {
        err = MMP_AUDIO_ERR_PARAMETER;
        m_AudioI2SConfig[ch].mclkMode = MMPS_AUDIO_I2S_256FS_MODE;
    }
    if ((I2SConfig->outputBitSize > MMPS_AUDIO_I2S_OUT_BITS_NONE) &&
        (I2SConfig->outputBitSize < MMPS_AUDIO_I2S_MAX_OUT_BIT_MODE)) {
        m_AudioI2SConfig[ch].outputBitSize = I2SConfig->outputBitSize;
    }
    else {
        err = MMP_AUDIO_ERR_PARAMETER;
        m_AudioI2SConfig[ch].outputBitSize = MMPS_AUDIO_I2S_OUT_16BIT;
    }
    if ((I2SConfig->inputBitSize > MMPS_AUDIO_I2S_IN_BITS_NONE) &&
        (I2SConfig->inputBitSize < MMPS_AUDIO_I2S_MAX_IN_BIT_MODE)) {
        m_AudioI2SConfig[ch].inputBitSize = I2SConfig->inputBitSize;
    }
    else {
        err = MMP_AUDIO_ERR_PARAMETER;
        m_AudioI2SConfig[ch].inputBitSize = MMPS_AUDIO_I2S_IN_16BIT;
    }
    if ((I2SConfig->lrckPolarity > MMPS_AUDIO_I2S_LRCK_POLARITY_NONE) &&
        (I2SConfig->lrckPolarity < MMPS_AUDIO_I2S_MAX_WS_POLARITY)) {
        m_AudioI2SConfig[ch].lrckPolarity = I2SConfig->lrckPolarity;
    }
    else {
        err = MMP_AUDIO_ERR_PARAMETER;
        m_AudioI2SConfig[ch].lrckPolarity = MMPS_AUDIO_I2S_LRCK_L_LOW;
    }
    if ((I2SConfig->outDataDelayMode > MMPS_AUDIO_I2S_OUT_DELAY_NONE) &&
        (I2SConfig->outDataDelayMode < MMPS_AUDIO_I2S_MAX_OUT_DELAY_MODE)) {
        m_AudioI2SConfig[ch].outDataDelayMode = I2SConfig->outDataDelayMode;
    }
    else {
        err = MMP_AUDIO_ERR_PARAMETER;
        if (m_AudioI2SConfig[ch].workingMode == MMPS_AUDIO_I2S_MASTER_MODE)
            m_AudioI2SConfig[ch].outDataDelayMode = MMPS_AUDIO_I2S_OUT_DELAY_STD;
        else
            m_AudioI2SConfig[ch].outDataDelayMode = MMPS_AUDIO_I2S_OUT_DELAY_I2S;
    }
    ...
    MMPD_AUDIO_SetI2SInBitWidth(ch, m_AudioI2SConfig[ch].inputBitSize);
...
/**
    ...
    AIT_AACPlus_Player_Init(MMP_BYTE *filename, MMP_BOOL bCardMode, DRM_INFO *drminfo)
    {
        ...
        MMPS_AUDIO_AACPLUS_INFO     aacplusinfo;
...
MMP_ERR MMPS_AUDIO_InitializeAACPlusPlay(MMPS_AUDIO_FILEINFO *fileinfo, MMPS_AUDIO_AACPLUS_INFO *aacplusinfo)
{
    ...
    MMPD_AUDIO_SetPlayFormat(MMPD_AUDIO_AACPLUS_PLAY);
    ...
    status = MMPD_AUDIO_GetAACPlusFileInfo((MMPD_AUDIO_AACPLUS_INFO *)aacplusinfo);
    if (status != MMP_ERR_NONE) {
        return status;
    }
...
MMP_ERR MMPS_AUDIO_InitializeFLACPlay(MMPS_AUDIO_FILEINFO *fileinfo, MMPS_AUDIO_FLAC_INFO *flacinfo)
{
    ...
    MMPD_AUDIO_SetOutPath(m_audioOutPath);
    MMPD_AUDIO_SetPlayFormat(MMPD_AUDIO_FLAC_PLAY);
    MMPD_AUDIO_EnableABRepeatMode(MMPS_AUDIO_AB_MODE_DISABLE);
    MMPD_AUDIO_SetPlayPosition(0);
    ...
    status = MMPD_AUDIO_GetFLACFileInfo((MMPD_AUDIO_FLAC_FILE_INFO *)flacinfo);
...
//==============================================================================
//  Function    : MMPS_AUDIO_FLACSeekFile
//==============================================================================
/** 
    @brief  FLAC Seek File(Memory mode)
    @param[in] *FileHandle : file handle pointer
    @param[in] ubAudioPtr : audio buffer start pointer
    @param[in] ulFileSize : file size
    @param[out] *ulReturnSize : actual transfer file size
    @return MMP_ERR_NONE
*/
/*
MMP_ERR MMPS_AUDIO_FLACSeekFile(void *FileHandle,MMP_UBYTE *ubAudioPtr,MMP_ULONG ulFileSize,MMP_ULONG *ulReturnSize)
{
    MMP_ULONG file_offset;
    MMP_USHORT start_flag,finish_flag;
    MMP_ULONG *FileId = (MMP_ULONG *)FileHandle;

    do{
        MMPD_AUDIO_GetPlayStartSeekFlag(&start_flag);
        if(start_flag==0x01){
            MMPD_AUDIO_GetPlayFileSeekOffset(&file_offset);
            MMPC_AUDIO_FileSeek(FileHandle,file_offset,0);
            MMPC_AUDIO_FileRead(FileHandle,ubAudioPtr,MMPS_AUDIO_DATA_BUF_SIZE,ulReturnSize);
            MMPD_AUDIO_UpdatePlayFileRemainSize(ulFileSize-file_offset);
            MMPD_AUDIO_InitPlayRWPtr();
            MMPS_AUDIO_TransferDataToAIT(ubAudioPtr,MMPS_AUDIO_DATA_BUF_SIZE,ulReturnSize);
...
//==============================================================================
//  Function    : MMPS_AUDIO_InitFLACPlayFromMem
//==============================================================================
/** 
    @brief  Initialize FLAC Play From Memory
    @param[in] *FileHandle : file  handle pointer
    @param[in] ubAudioPtr : audio data start pointer
    @param[in] ulFileSize : file size
    @param[out] *fileinfo : flac file info 
    @return MMP_ERR_NONE,MMP_AUDIO_ERR_UNSUPPORT_FORMAT
    
    @remarks Modify MMPC_AUDIO_FileRead() and MMPC_AUDIO_FileSeek() before using this function
*/
/*
MMP_ERR MMPS_AUDIO_InitFLACPlayFromMem(void *FileHandle,MMP_UBYTE *ubAudioPtr,MMP_ULONG ulFileSize,MMPS_AUDIO_FLAC_INFO *fileinfo)
{
    ...
    MMPD_AUDIO_InitPlayRWPtr();
    MMPD_AUDIO_SetFLACPlayTime(0);
    MMPD_AUDIO_TransferDataToAIT(ubAudioPtr,ulFileSize,&return_size);

#if 1 //samyu: oggflac
    MMPD_AUDIO_InitPlaySeekFlag();   
    MMPD_AUDIO_SendGetFLACFileInfoCmd();
    MMPS_AUDIO_FLACSeekFile(FileHandle,ubAudioPtr,ulFileSize,&return_size);
    MMPD_AUDIO_GetFLACFileInfo_MemMode((MMPD_AUDIO_FLAC_FILE_INFO *)fileinfo);
#else
    MMPD_AUDIO_GetFLACFileInfo((MMPD_AUDIO_FLAC_INFO *)fileinfo);
#endif
...
*/
//==============================================================================
//  Function    : MMPS_AUDIO_StartFLACPlay_MemMode
//==============================================================================
/** 
    @brief  Start FLAC Play From Memory
    @param[in] *FileHandle : file handle pointer
    @param[in] ubAudioPtr : audio buffer start pointer
    @param[in] ulFileSize : file size
    @param[out] *ulReturnSize : actual transfer file size
...
*/
/*
MMP_ERR MMPS_AUDIO_StartFLACPlay_MemMode(void *FileHandle,MMP_UBYTE *ubAudioPtr,MMP_ULONG ulFileSize,MMP_ULONG *ulReturnSize)
{
    MMP_ERR status;
   
    MMPD_AUDIO_InitPlaySeekFlag();
    
    status = MMPD_AUDIO_StartFLACPlay_MemMode();
    
    if(status==MMP_AUDIO_ERR_COMMAND_INVALID){
        return status;
    }   

    MMPS_AUDIO_FLACSeekFile(FileHandle,ubAudioPtr,ulFileSize,ulReturnSize);
...
MMP_ERR MMPS_AUDIO_InitMIDIPlayFromCard(MMPS_AUDIO_FILEINFO *fileinfo, MMPS_AUDIO_MIDI_INFO *midiinfo)
{
    ...
    MMPD_AUDIO_SetOutPath(m_audioOutPath);
    MMPD_AUDIO_SetPlayFormat(MMPD_AUDIO_MIDI_PLAY);
    MMPD_AUDIO_EnableABRepeatMode(MMPS_AUDIO_AB_MODE_DISABLE);
    MMPD_AUDIO_SetPlayPosition(0);
    MMPD_AUDIO_SetPlayPath(MMPD_AUDIO_PLAY_FROM_CARD);
    ...
    MMPD_AUDIO_GetMIDIFileInfo((MMPD_AUDIO_MIDI_FILE_INFO *)midiinfo);
...
MMP_ERR MMPS_AUDIO_InitMIDIPlayFromMem(MMP_UBYTE *ubAudioPtr,MMP_ULONG ulFileSize, MMPS_AUDIO_MIDI_INFO *fileinfo)
{
    ...
    MMPD_AUDIO_GetMIDIFileInfo((MMPD_AUDIO_MIDI_FILE_INFO *)fileinfo);
    if (fileinfo->usNotSupport) {
        return MMP_AUDIO_ERR_UNSUPPORT_FORMAT;
    }
...
MMP_ERR MMPS_AUDIO_InitializeMP3Play(MMPS_AUDIO_FILEINFO *fileinfo, MMPS_AUDIO_MP3_INFO *mp3info)
{
    ...
    MMPD_AUDIO_SetParseFormat(MMPD_AUDIO_MP3_PLAY);
    if (m_audioPath == MMPS_AUDIO_MEDIA_PATH_CARD) {
    ...
    MMPD_AUDIO_GetMP3FileInfo((MMPD_AUDIO_MP3_INFO *)mp3info);
...
MMP_ERR MMPS_AUDIO_ParserGetMp3Info(MMPS_AUDIO_FILEINFO *fileinfo, MMPS_AUDIO_MP3_INFO *mp3info)
{
	...
	if (ret)
		return ret;
    ret = MMPD_AUDPSR_GetMp3Info((MMPD_AUDIO_MP3_INFO *)mp3info);
...
MMP_ERR MMPS_AUDIO_ParserGetAACPlusInfo(MMPS_AUDIO_FILEINFO *fileinfo, MMPS_AUDIO_AACPLUS_INFO *aacplusinfo)
{
	...
	if (ret)
		return ret;
    ret = MMPD_AUDPSR_GetAACPlusInfo((MMPD_AUDIO_AACPLUS_INFO *)aacplusinfo);
	if (fileinfo->usFileNameLength)
	    MMPD_AUDPSR_Finish(MMP_TRUE);
    else
        MMPD_AUDPSR_Finish(MMP_FALSE); //don't need to close file
...
MMP_ERR MMPS_AUDIO_ParserGetWMAInfo(MMPS_AUDIO_FILEINFO *fileinfo, MMPS_AUDIO_WMA_INFO *wmainfo)
{
	...
	if (ret)
		return ret;
    ret = MMPD_AUDPSR_GetWMAInfo((MMPD_AUDIO_WMA_INFO *)wmainfo);
	if (fileinfo->usFileNameLength)
	    MMPD_AUDPSR_Finish(MMP_TRUE);
    else
        MMPD_AUDPSR_Finish(MMP_FALSE); //don't need to close file
...
MMP_ERR MMPS_AUDIO_ParserGetWAVInfo(MMPS_AUDIO_FILEINFO *fileinfo, MMPS_AUDIO_WAV_INFO *wavinfo)
{
    ...
    if (ret)
        return ret;
    ret = MMPD_AUDPSR_GetWAVInfo((MMPD_AUDIO_WAV_INFO *)wavinfo);
    if (fileinfo->usFileNameLength)
	    MMPD_AUDPSR_Finish(MMP_TRUE);
    else
        MMPD_AUDPSR_Finish(MMP_FALSE); //don't need to close file
...
MMP_ERR MMPS_AUDIO_ParserGetOGGInfo(MMPS_AUDIO_FILEINFO *fileinfo, MMPS_AUDIO_OGG_INFO *ogginfo)
{
	...
	if (ret)
		return ret;
    ret = MMPD_AUDPSR_GetOGGInfo((MMPD_AUDIO_OGG_INFO *)ogginfo);
	if (fileinfo->usFileNameLength)
	    MMPD_AUDPSR_Finish(MMP_TRUE);
    else
        MMPD_AUDPSR_Finish(MMP_FALSE); //don't need to close file
...
MMP_ERR MMPS_AUDIO_ParserGetRAInfo(MMPS_AUDIO_FILEINFO *fileinfo, MMPS_AUDIO_RA_INFO *rainfo)
{
	...
	if (ret)
		return ret;
    ret = MMPD_AUDPSR_GetRAInfo((MMPD_AUDIO_RA_INFO *)rainfo);
	if (fileinfo->usFileNameLength)
	    MMPD_AUDPSR_Finish(MMP_TRUE);
    else
        MMPD_AUDPSR_Finish(MMP_FALSE); //don't need to close file
...
MMP_ERR MMPS_AUDIO_ParserGetAMRInfo(MMPS_AUDIO_FILEINFO *fileinfo, MMPS_AUDIO_AMR_INFO *amrinfo)
{
    ...
    if (ret)
        return ret;
    ret = MMPD_AUDPSR_GetAMRInfo((MMPD_AUDIO_AMR_INFO *)amrinfo);
    if (fileinfo->usFileNameLength)
	    MMPD_AUDPSR_Finish(MMP_TRUE);
    else
        MMPD_AUDPSR_Finish(MMP_FALSE); //don't need to close file
...
MMP_ERR MMPS_AUDIO_InitializeWMAPlay(MMPS_AUDIO_FILEINFO *fileinfo, MMPS_AUDIO_WMA_INFO *wmainfo)
{
    ...
    MMPD_AUDIO_SetOutPath(m_audioOutPath);
    MMPD_AUDIO_SetPlayFormat(MMPD_AUDIO_WMA_PLAY);
    MMPD_AUDIO_EnableABRepeatMode(MMPS_AUDIO_AB_MODE_DISABLE);
    MMPD_AUDIO_SetPlayPosition(0);
	...
	MMPD_AUDIO_GetWMAFileInfo((MMPD_AUDIO_WMA_INFO *)wmainfo);
...
MMP_ERR MMPS_AUDIO_InitializeRAPlay(MMPS_AUDIO_FILEINFO *fileinfo, MMPS_AUDIO_RA_INFO *rainfo)
{
    ...
    MMPD_AUDIO_SetOutPath(m_audioOutPath);
    MMPD_AUDIO_SetPlayFormat(MMPD_AUDIO_RA_PLAY);
    MMPD_AUDIO_EnableABRepeatMode(MMPS_AUDIO_AB_MODE_DISABLE);
    MMPD_AUDIO_SetPlayPosition(0);
	...
	MMPD_AUDIO_GetRAFileInfo((MMPD_AUDIO_RA_INFO *)rainfo);
...
MMP_ERR MMPS_AUDIO_InitializeOGGPlay(MMPS_AUDIO_FILEINFO *fileinfo, MMPS_AUDIO_OGG_INFO *ogginfo)
{
    ...
    MMP_ULONG64 filesize;
    MMP_ULONG   readsize;
    MMPD_AUDIO_OGG_TAG_INFO taginfo;
	...
	MMPD_AUDIO_SetOutPath(m_audioOutPath);
    MMPD_AUDIO_SetPlayFormat(MMPD_AUDIO_OGG_PLAY);
    MMPD_AUDIO_EnableABRepeatMode(MMPS_AUDIO_AB_MODE_DISABLE);
    MMPD_AUDIO_SetPlayPosition(0);
    if (m_audioPath == MMPS_AUDIO_MEDIA_PATH_CARD) {
        MMPD_AUDIO_SetPlayPath(MMPD_AUDIO_PLAY_FROM_CARD);
    ...
    MMPD_AUDIO_GetOGGFileInfo((MMPD_AUDIO_OGG_INFO *)ogginfo);
    if (m_audioPath == MMPS_AUDIO_MEDIA_PATH_CARD) {
        // Must call MMPD_AUDIO_GetOGGFileInfo() first
        MMPD_AUDIO_GetOGGTagInfo((MMPD_AUDIO_OGG_TAG_INFO *)&taginfo);
    }
...
MMP_ERR MMPS_AUDIO_InitializeWAVPlay(MMPS_AUDIO_FILEINFO *fileinfo, MMPS_AUDIO_WAV_INFO *wavinfo)
{
    ...
    MMPD_AUDIO_SetOutPath(m_audioOutPath);
    MMPD_AUDIO_SetPlayFormat(MMPD_AUDIO_WAV_PLAY);
    MMPD_AUDIO_EnableABRepeatMode(MMPS_AUDIO_AB_MODE_DISABLE);
    MMPD_AUDIO_SetPlayPosition(0);
    ...
    MMPD_AUDIO_GetWAVFileInfo((MMPD_AUDIO_WAV_INFO *)wavinfo);
...
MMP_ERR MMPS_AUDIO_InitializeAMRPlay(MMPS_AUDIO_FILEINFO *fileinfo, MMPS_AUDIO_AMR_INFO *amrinfo)
{
    ...
    #endif
    MMPD_AUDIO_SetOutPath(m_audioOutPath);
    MMPD_AUDIO_SetPlayFormat(MMPD_AUDIO_AMR_PLAY);
    MMPD_AUDIO_EnableABRepeatMode(MMPS_AUDIO_AB_MODE_DISABLE);
    MMPD_AUDIO_SetPlayPosition(0);
    ...
    MMPD_AUDIO_GetAMRFileInfo((MMPD_AUDIO_AMR_INFO *)amrinfo);
...
{
	return MMPD_AUDIO_SetRecordAnalogGain(ubAGain, bboostup);
}
...
{
    return MMPD_AUDIO_SetRecordVolume(ubDGain, ubAGain, bboostup);
}
...
MMP_ERR MMPS_AUDIO_InitAMRRecordToCard(MMPS_AUDIO_FILEINFO *fileinfo, MMPS_AUDIO_AMR_ENCODEMODE mode,MMP_ULONG ulMaxSize)
{
    ...
    MMPD_AUDIO_SetEncodeFormat(MMPD_AUDIO_AMR_ENCODE);
...
MMP_ERR MMPS_AUDIO_InitAACRecordToCard(MMPS_AUDIO_FILEINFO *fileinfo, MMPS_AUDIO_AAC_ENCODEMODE mode,MMP_ULONG ulMaxSize)
{
    ...
    MMPD_AUDIO_SetEncodeFileName(fileinfo->bFileName, fileinfo->usFileNameLength);
    MMPD_AUDIO_SetEncodeFormat(MMPD_AUDIO_AAC_ENCODE);
    MMPD_AUDIO_SetEncodeMode(mode);
...
MMP_ERR MMPS_AUDIO_InitMP3RecordToCard(MMPS_AUDIO_FILEINFO *fileinfo, MMPS_AUDIO_MP3_ENCODEMODE mode, MMP_ULONG ulMaxSize)
{
    ...
    if (err != MMP_ERR_NONE)
        return err;
    MMPD_AUDIO_SetEncodeFormat(MMPD_AUDIO_MP3_ENCODE);
    MMPD_AUDIO_SetEncodeMode(mode);
...
MMP_ERR MMPS_AUDIO_InitWAVRecord(MMPS_AUDIO_FILEINFO *fileinfo, MMPS_AUDIO_WAV_ENCODEMODE wav_encode_mode, MMP_ULONG ulMaxSize)
{
    ...
    MMPD_AUDIO_SetEncodeFormat(MMPD_AUDIO_WAV_ENCODE);
...
MMP_ERR MMPS_AUDIO_InitializePCMPlay(MMPS_AUDIO_FILEINFO *fileinfo)
{
    ...
    MMPD_AUDIO_SetOutPath(m_audioOutPath);
    MMPD_AUDIO_SetPlayFormat(MMPD_AUDIO_PCM_PLAY);
    MMPD_AUDIO_EnableABRepeatMode(MMPS_AUDIO_AB_MODE_DISABLE);
    MMPD_AUDIO_SetPlayPosition(0);
...
MMP_ERR MMPS_AUDIO_OpenFile(MMPS_AUDIO_FILEINFO	fileinfo, void* ContainerInfo, MMPS_AUDIO_CODEC AudioFormat) 
{
    ...
    case MMPS_AUDIO_CODEC_MP3:
        m_Mp3Info = (MMPS_AUDIO_MP3_INFO*)ContainerInfo;
        ...
        if (m_Mp3Info->usNotSupport==1) {
            ...
            //RTNA_DBG_Str(0, "Unsupport Format\r\n");
        }
        else {
            //RTNA_DBG_Str(0, "Support Format\r\n");
        }                    
        break;
    case MMPS_AUDIO_CODEC_WAV:
        m_WavInfo = (MMPS_AUDIO_WAV_INFO*)ContainerInfo;
    ...
    case MMPS_AUDIO_CODEC_WMA:
        m_WMAproInfo = (MMPS_AUDIO_WMA_INFO*)ContainerInfo;
    ...
    case MMPS_AUDIO_CODEC_AAC:
        m_AacplusInfo = (MMPS_AUDIO_AACPLUS_INFO*)ContainerInfo;
    ...
    case MMPS_AUDIO_CODEC_OGG:
        m_OggInfo = (MMPS_AUDIO_OGG_INFO*)ContainerInfo;
    ...
    case MMPS_AUDIO_CODEC_RA:
        m_RaInfo = (MMPS_AUDIO_RA_INFO*)ContainerInfo;
    ...
    case MMPS_AUDIO_CODEC_FLAC:
        m_FLACInfo = (MMPS_AUDIO_FLAC_INFO*)ContainerInfo;
...
MMP_ERR MMPS_AUDIO_SeekByTime(MMP_LONG TargetTime) 
{
    ...
    #if (GAPLESS_PLAY_EN == 1)
    MMPS_AUDIO_ChkOperationAllowed(MMPS_AUDIO_GAPLESS_SEEK, &bOpAllowed);
    if (!bOpAllowed) {
...
MMP_ERR  MMPS_AUDIO_OpenMusicListFile(void)
{
    ...
    PRINTF("Content Struct Size:%X\r\n",sizeof(MMPS_AUDIO_MUSICLIST_CONTENT));
    PRINTF("Obj Cnt:%d\r\n",m_MusicListListInfo.ObjCnt);
    ...
    for(i=0;i<m_MusicListListInfo.ObjCnt;i++){
        ...
        //PRINTF("%X\r\n",m_MusicListListInfo.ListContent.FileFmt);
    }
...
MMP_ERR MMPS_AUDIO_GetCurCodec(MMP_UBYTE* ubcurCodec)
{
	...
	else *ubcurCodec = MMPS_AUDIO_INTERNAL_CODEC;
	
	return MMP_ERR_NONE;
	//add hdmi condition here;
}
...
//------------------------------------------------------------------------------
//  Function    : MMPS_AUDIO_GetRecordStatus
//------------------------------------------------------------------------------
/**
    @brief  Get Audio Recorder Status
    @param[out] status : record status
        -   MMPS_AUDIO_RECORD_START
        -   MMPS_AUDIO_RECORD_PAUSE
        -   MMPS_AUDIO_RECORD_STOP
    @return MMP_ERR_NONE
*/
MMP_ERR MMPS_AUDIO_GetRecordStatus(MMPS_AUDIO_RECORD_STATUS *status)
{
    return MMPD_AUDIO_GetRecordStatus((MMP_USHORT *)status);
}
...
/**
    ...
    @return MMP_ERR_NONE
*/
MMP_ERR MMPS_AUDIO_InitLiveRecord(  MMPS_AUDIO_ENCODE_FMT   fmt,
                                    MMP_USHORT              mode,
...
{
    MMPD_AUDIO_ENCODE_FMT   fmt;
    MMP_ERR                 error = MMP_ERR_NONE;
    
    MMPD_AUDIO_GetEncodeFormat(&fmt);
    
    switch (fmt) {
		case MMPD_AUDIO_AMR_ENCODE:
			error = MMPS_AUDIO_StartAMRRecord();
		break;
		case MMPD_AUDIO_AAC_ENCODE:
			error = MMPS_AUDIO_StartAACRecord();
		break;
		case MMPD_AUDIO_MP3_ENCODE:
			error = MMPS_AUDIO_StartMP3Record();
		break;
		case MMPD_AUDIO_WAV_ENCODE:
...
MMP_ERR MMPS_LiveAudio_StartRecord(void)
{
    ...
    switch (fmt) {
    case MMPD_AUDIO_AAC_ENCODE:
	case MMPD_AUDIO_G711_ENCODE:
    case MMPD_AUDIO_PCM_ENCODE:
...
{
	#if 0 //debug usage
	VirtualBassEQHandler* tmp;
	
	tmp = argHandler;
	
	RTNA_DBG_PrintLong(0, tmp->SampleRate);
	RTNA_DBG_PrintLong(0, tmp->nCh);
	RTNA_DBG_PrintLong(0, tmp->Fc);
	RTNA_DBG_PrintLong(0, tmp->Gain);
	RTNA_DBG_PrintLong(0, tmp->HPFEnable);
    #endif 
...
*/
MMP_ERR MMPS_AUDIO_ConfigWNR(MMPS_AUDIO_WNR_CFG *cfg)
{
