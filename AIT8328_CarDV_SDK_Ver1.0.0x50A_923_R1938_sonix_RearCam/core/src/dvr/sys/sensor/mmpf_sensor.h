...
#if ((VIDEO_R_EN)||(DSC_R_EN))&&(SUPPORT_FDTC)
#include "mmpf_fdtc.h"
#endif
#include "mmpf_vif.h"
...
typedef struct _MMPF_SENSOR_CUSTOMER {
    ...
    void (*MMPF_Cust_CheckSnrTVSrcType)(MMP_UBYTE ubSnrSel, MMPF_VIF_TV_DEC_SRC_TYPE *TVSrcType);
...
typedef struct _MMPF_SENSOR_FUNCTIION 
{
    ...
    MMP_ERR (*MMPF_Sensor_CheckSnrTVSrcType)(MMP_UBYTE ubSnrSel, MMPF_VIF_TV_DEC_SRC_TYPE *TVSrcType);
...
void MMPF_ISP_ISPOperationDonePost(void);
/* ISP Function */
...
typedef void        (MMPF_SENSOR_Callback) (void);
typedef void        (MMPF_SENSOR_EventCallback) (MMP_ULONG ulPara1, MMP_ULONG ulPara2);

void 	MMPF_Sensor_GetTV_Dec_Src_Type(MMPF_VIF_TV_DEC_SRC_TYPE *sSnrTVSrc);
MMP_ERR MMPF_Sensor_RegisterCallback(MMPF_SENSOR_EVENT ulEvent, void *pCallback);
void MMPF_Sensor_CheckTV_Dec_Src_Type(MMPF_VIF_TV_DEC_SRC_TYPE *sSnrTVSrc);
void MMPF_Sensor_EnableTV_Dec_Src_TypeDetection(MMP_BOOL bEnableDetection);
