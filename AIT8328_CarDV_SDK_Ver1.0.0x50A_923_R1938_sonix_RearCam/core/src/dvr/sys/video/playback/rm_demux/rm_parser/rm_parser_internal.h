...
/* AIT Seek/Search function define */
#define AIT_RM_SEEKTONEXTKEY       0x00000001
#define AIT_RM_SEEKTONEARESTKEY    0x00000002
#define AIT_RM_SEEKVIDEO           0x00010000
#define AIT_RM_SEEKAUDIO           0x00020000
#define AIT_RM_FORWARDSEARCH       0x00000100
#define AIT_RM_BACKWARDSEARCH      0x00000200
...
struct rm_stream_info
{
   ...
   HX_BITFIELD               bIsRealAudio              ;
   HX_BITFIELD               bIsRealVideo              ;
   HX_BITFIELD               bIsRealEvent              ;
   HX_BITFIELD               bStreamDone               ;
   HX_BITFIELD               bNeedKeyframe             ;
   HX_BITFIELD               bSeenFirstPacketTimeStamp ;
};
...
typedef struct rm_parser_internal_struct
{
    ...
    // Will Add
    struct rm_index_hdr           idxHdr;
    UINT32                        ulHdrAvailable;
...
int rm_parseri_set_buf(BYTE *addr);
...
HX_RESULT rm_parseri_read_next_packet(rm_parser_internal* pInt, rm_packet** ppPacket, UINT32 ulDecStreamNum);
...
HX_RESULT rm_parseri_search_all_seek_tables(rm_parser_internal* pInt, UINT32 ulSeekTime,
                                            UINT32* pulFoundTime, UINT32* pulFoundOffset, UINT32 ulFlag);
HX_RESULT rm_parseri_search_seek_table(struct rm_seek_table* pTable, UINT32 ulSeekTime,
                                       UINT32* pulFoundTime, UINT32* pulFoundOffset, UINT32 ulFlag);
HX_RESULT rm_parseri_search_index_chunk(rm_parser_internal* pInt, UINT32 *ulSeekTime, UINT32 ulFlag);
HX_RESULT rm_parseri_seek(rm_parser_internal* pInt, UINT32 *ulSeekTime, UINT32 ulFlag);
