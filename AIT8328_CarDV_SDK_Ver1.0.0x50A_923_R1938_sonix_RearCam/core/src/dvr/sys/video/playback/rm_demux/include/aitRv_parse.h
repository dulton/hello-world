/* ***** BEGIN LICENSE BLOCK *****
 * Source last modified: $Id: aitRv_rm_memory.h,v 1.1.1.1.2.1 2005/05/04 18:20:57 hubbe Exp $
 * 
...
//typedef UINT32 (*rm_read_func_ptr) (
//                                    BYTE*   pBuf, /* Must be at least ulBytesToRead long */
//                                    UINT32  ulBytesToRead);
//typedef void (*rm_seek_func_ptr) (
//                                  UINT32 ulOffset,
//                                  UINT32 ulOrigin);
...
int aitRv_parser_set_buf(BYTE *addr);
/*
 * aitRv_parser_is_rm_file()
 *
 ...
 */
HXBOOL aitRv_parser_is_rm_file(BYTE* pBuf, UINT32 ulSize);
 ...
 * return value.
 */
rm_parser* aitRv_parser_create(void*             pError,
                            rm_error_func_ptr fpError);

rm_parser* aitRv_parser_dup(rm_parser* fpParser);

/*
 * aitRv_parser_create2
 *
 ...
 */
rm_parser* aitRv_parser_create2(void*              pError,
                             rm_error_func_ptr  fpError,
                             void*              pMem,
...
/*
 * aitRv_parser_init_io
 *
 * If the user didn't want to use stdio, they would call
 * this function to set up their own I/O.
 */
HX_RESULT aitRv_parser_init_io(rm_parser*        pParser,
                            void*             pUserRead,
                            rm_read_func_ptr  fpRead,
                            rm_seek_func_ptr  fpSeek);


unsigned int aitRv_parser_get_indexoffset(rm_parser* pParser, unsigned int file_size);

/*
 * aitRv_parser_read_headers
 *
 ...
 */
HX_RESULT aitRv_parser_read_headers(rm_parser* pParser);
 ...
 * is present by calling aitRv_parser_get_title_length(). This length
 * is the length in bytes of the string INCLUDING the null
 * terminator. If the title meta-data IS present, then the user
 * can get read-only access to the title by calling aitRv_parser_get_title().
 ...
 */
UINT32      aitRv_parser_get_max_bit_rate(rm_parser* pParser);
UINT32      aitRv_parser_get_avg_bit_rate(rm_parser* pParser);
UINT32      aitRv_parser_get_max_packet_size(rm_parser* pParser);
UINT32      aitRv_parser_get_avg_packet_size(rm_parser* pParser);
UINT32      aitRv_parser_get_num_packets(rm_parser* pParser);
UINT32      aitRv_parser_get_duration(rm_parser* pParser);
UINT32      aitRv_parser_get_preroll(rm_parser* pParser);
const char* aitRv_parser_get_title(rm_parser* pParser);
const char* aitRv_parser_get_author(rm_parser* pParser);
const char* aitRv_parser_get_copyright(rm_parser* pParser);
const char* aitRv_parser_get_comment(rm_parser* pParser);
UINT32      aitRv_parser_get_num_streams(rm_parser* pParser);
 ...
 * present in the file.
 */
HX_RESULT aitRv_parser_get_file_properties(rm_parser*    pParser,
                                        rm_property** ppProp,
                                        UINT32*       pulNumProps);

/*
 * This function destroys an array of properties created
 * by either aitRv_parser_get_file_properties or
 * by rm_parser_get_stream_properties.
 */
void aitRv_parser_destroy_properties(rm_parser*    pParser,
                                  rm_property** ppProp,
                                  UINT32*       pulNumProps);

/*
 * aitRv_parser_get_stream_header
 *
 ...
 */
HX_RESULT aitRv_parser_get_stream_header(rm_parser*         pParser,
                                      UINT32             ulStreamNum,
                                      rm_stream_header** ppHdr);

/*
 * aitRv_parser_destroy_stream_header
 *
 * This function frees any memory associated with
 * a stream header obtained by calling aitRv_parser_get_stream_header().
 */
void aitRv_parser_destroy_stream_header(rm_parser*         pParser,
                                     rm_stream_header** ppHdr);

/*
 * aitRv_parser_get_packet
 *
 * After the headers had been read, the user can repeatedly
 * call this function to retrieve packets from the .rm file.
 */
HX_RESULT aitRv_parser_get_packet(rm_parser*  pParser,
                               rm_packet** ppPacket,
                               UINT32 ulStreamNum);

/*
 * aitRv_parser_destroy_packet
 *
 * This is called to clean up packets created by
 * calls to aitRv_parser_get_packet().
 */
void aitRv_parser_destroy_packet(rm_parser*  pParser,
                              rm_packet** ppPacket);
 ...
 */
HX_RESULT aitRv_parser_seek(rm_parser* pParser,
                         UINT32     *ulTime,
                         HXBOOL     bNextKey,
                         HXBOOL     bNearest,
                         HXBOOL     bSeekAudio);

HX_RESULT aitRv_parser_resetsearch(rm_parser* pParser);
                         
HX_RESULT aitRv_parser_searchKey(rm_parser* pParser,
                         UINT32     *ulTime, 
                         HXBOOL     bSearchBack,
                         UINT32     ulLoop);

/*
 * aitRv_parser_destroy
 *
 * The user calls this to clean up
 */
void aitRv_parser_destroy(rm_parser** ppParser);
