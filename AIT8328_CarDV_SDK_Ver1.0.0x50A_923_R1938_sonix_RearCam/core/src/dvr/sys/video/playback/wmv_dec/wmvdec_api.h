...
Abstract:
    ...
    Decoder API's for WMV.
        decodes WMV3, WMV2, WMV1, MP43, MP42, MP4S
...
#define	wmvdec_assert(e)	//{if(!(e)){RTNA_DBG_Str(0,__FILE__);RTNA_DBG_Long(0,__LINE__);RTNA_DBG_Str(0, "\r\n");}}
...
#else
typedef struct tagWMVSyncAV {
    int             m_iMSOffset       ;   // time offset between when last frame was complete and the ideal display time.  In milliseconds, negative is behind.
    tWMVAVSyncState m_eAVSyncState    ;   // tell codec how A/V sync is going: better user experience if this is implemented.
    unsigned int    m_uiReserved      ;   // all 0 or 1 ignored (all other values reserved)
    tWMVPostLevel   m_ePostLevel      ;   // set post level, recommend WMVPostAuto
} WMVSyncAV;
#endif
...
#define FOURCC_ASyncVer_Win32   mmioFOURCC_WMV('a','v','0','1')
...
#define FOURCC_ASyncVer_Win32   'av01'
...
#define FOURCC_NV12_WMV             0x3231564E
...
#ifdef XBOXGPU
    tWMVDecodeStatus GPUWMVideoDecInit (
        HWMVDecoder* phWMVDecoder,
        U32_WMV uiUserData,
        U32_WMV uiFOURCCCompressed,
        Float_WMV fltFrameRate,
        ...
        U32_WMV uipSequenceHeaderLength,
        Bool_WMV bHostDeinterlace,
        I32_WMV iBufferCount,
        I32_WMV iFirstFrameDelay,
        I32_WMV iFrameDelay
    ) ;
    I32_WMV GPUWMVideoGetLastFrameDelay(HWMVDecoder* phWMVDecoder);
    Void_WMV GPUWMVSetFrameDealy(HWMVDecoder* phWMVDecoder,I32_WMV iFrameDelay);
#endif
...
tWMVDecodeStatus WMVideoDecDecodeData (HWMVDecoder hWMVDecoder, U16_WMV* puiNumDecodedFrames, WMVSyncAV avSyncInfo);


tWMVDecodeStatus WMVideoDecGetFrameType (HWMVDecoder hWMVDecoder, U32_WMV * pFrameType);

// uiWidthBMPOutput allows the wourse width to be output into a larger BMP of width uiWidthBMPOutput.
...
// *** *** *** *** *** ***
// This functiuon returns the internal frame rate extracted from the raw bitstream.  Not a substitute for timestamps.  Callable after WMVideoDecDecodeSequenceHeader().
...
//This function is used by an app that wants to do an software based display resize. 
