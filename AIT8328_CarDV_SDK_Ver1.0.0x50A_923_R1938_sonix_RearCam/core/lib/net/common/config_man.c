...
{
	int i;
	struct amn_config_space_handler_t *hand = &amn_ConfigSpaceHandler[0];
...
int amn_config_wlocate( char *key )
{
	...
	last_key = NULL;  // reset for new search
	return -1;
}
...
unsigned int export_net_config_file( int type, char* path )
{
	...
	if(fileID == NULL) {
		osal_dprint(WARN,"Export: Open Config File Fail. File:%s\r\n", path);
		return 1;
	}
	for(i = 0; i < AMN_CONFIGSPACE_TOTAL_ENTRIES; i++) {
		curcfg = amn_config_get(type, amn_ConfigSpaceDict[i]);
		...
		if(ls!=os) {
			osal_dprint(WARN,"Export: Write Config File Fail. File:%s\r\n", path);
			osal_dprint(WARN,"Export String:%s\r\n", line);
			osal_dprint(WARN,"Export Size:%d, Write Size:%d\r\n", ls, os);
			err = 1;
		}
	}
	osal_dprint(INFO,"Export Config File:%s\r\n", path);
	osal_fclose(fileID);
	{
		...
		while (ptr) {
			...
			*ptr++ = 0;
			...
			if(amn_config_set(type, key, val)) {
				osal_dprint(WARN ,"Error NetConf: key=%s, val=%s\r\n",
										key, val);
			}
			...
			else
			{
				...
				switch(cfg->type) {
					case CFGSPACE_VALTYP__INT:
						osal_dprint(DEBUG,"Read val=%d\r\n", cfg->v.intVal);
						break;
					case CFGSPACE_VALTYP__STR:
						osal_dprint(DEBUG,"Read String=%s\r\n", cfg->v.strVal);
						break;
				}
			}
#endif
...
unsigned char apply_user_net_setting(int type, char* path)
{
	...
	if(fileId == NULL)
	{
		osal_dprint(WARN, "No User Network Setting File. File:%s, Type:%d\r\n", path, type);
		return 1;
	}	
	...
	if(status)
	{
		osal_dprint(WARN, "Apply Network User Setting Failed. File:%s, Type:%d\r\n", path, type);
		osal_fclose(fileId);
		return 1;
	}	
	osal_fclose(fileId);
