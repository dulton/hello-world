...
#include "wmconstants.h"

// The following are ANSI C includes, and so should be safe for all platforms (end)
// This is a walk around to turn on WMA_ASSERT in build environment. Since we are
// not allowed to have debug bits CRT, we don't have DEBUG_CRT defined and NDEBUG is
// defined in makefile.def. Later in WMA_ASSERT.h WMA_ASSERT is defined as void. But since
...
//#endif
//#include <WMA_ASSERT.h>     // If this doesn't exist, you must write an "WMA_ASSERT" macro
...
static INLINE I64 multHi64(I64 a, I32 b, Int bits)
{
  if (bits <= 32) {
    if (a > 0)
...
static INLINE I64 multHi64B31(I64 a, I32 b)
{
...
static INLINE I64 multHi64B30(I64 a, I32 b)
{
#if 0
  U64 r;
  U64 s = a >> 63;
...
static INLINE I32 wmaiiround2toN(const I32 x, const I32 N) {
...
#define FLOATORINT_ASSERT(x)                WMA_ASSERT(x)
...
#endif
{
    ...
    WMA_ASSERT (pbSrc != NULL && pbDst != NULL);
    WMA_ASSERT (iBitStartSrc < BITS_PER_DWORD && iBitStartSrc >= 0);
    WMA_ASSERT (cBits >= 0);
...
static Float FloatFromFastFloat( FastFloat fflt )
    {
        WMA_ASSERT( 0<= fflt.iFracBits && fflt.iFracBits <= 50 );
