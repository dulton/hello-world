...
#if (CHIP == P_V2)
#define  PIP_FB0                0x01320000 //Rogers@JWD:
#endif
...
#if 0 // Using AIT's graphic OPR to fill RGB pattern
{
    ...
    LCD_SET_D(LCD_MISC_BG_COLR,    0x00000000 );

    winattribute.usWidth = g_PIP_w;
    winattribute.usHeight = g_PIP_h;   
    winattribute.usLineOffset = g_PIP_w * g_PIP_bpp;
    winattribute.colordepth = MMPD_DISPLAY_WINCOLORDEPTH_16;
    winattribute.ulBaseAddr = PIP_FB0;
    MMPD_Display_SetWinAttributes(MMPD_DISPLAY_WIN_PIP, &winattribute);

    MMPD_Display_SetWinTransparent(MMPD_DISPLAY_WIN_PIP, 1, 0);

    MMPD_Display_SetWinPriority(MMPD_DISPLAY_WIN_MAIN, 
                                MMPD_DISPLAY_WIN_PIP,
                                MMPD_DISPLAY_WIN_OVERLAY,
                                MMPD_DISPLAY_WIN_ICON);

    dispattribute.usStartX = 0;
    dispattribute.usStartY = 0;
    dispattribute.usDisplayWidth = g_PIP_w;   
    dispattribute.usDisplayHeight = g_PIP_h;   
    dispattribute.usDisplayOffsetX = 0;   
    dispattribute.usDisplayOffsetY = 0;   
    dispattribute.bMirror = 0;   
    dispattribute.rotatetype = MMPD_DISPLAY_ROTATE_NO_ROTATE;   
    MMPD_Display_SetWinToDisplay(MMPD_DISPLAY_WIN_PIP, &dispattribute);
    MMPD_Display_SetWinActive(MMPD_DISPLAY_WIN_PIP, 1);



//#ifdef BUILD_HOST
//MMPH_HIF_RegGetL(0x7200);
//#endif
    ...
    // LCD Scaling Setting
    LCD_SET_W(LCD_SIN_W,               g_PIP_w);
    LCD_SET_W(LCD_SIN_H,               g_PIP_h);
    LCD_SET_D(LCD_SOUT_GRAB_PIXL_CNT,  g_PIP_w * g_PIP_h);
    LCD_SET_W(LCD_SOUT_GRAB_H_ST,      1);
    LCD_SET_W(LCD_SOUT_GRAB_H_ED,      g_PIP_w);
    LCD_SET_W(LCD_SOUT_GRAB_V_ST,      1);
    LCD_SET_W(LCD_SOUT_GRAB_V_ED,      g_PIP_h);


    // preview
    LCD_SET_D(LCD_PIP_1_ADDR_ST,   PIP_FB1);


    bufattribute.usWidth = g_PIP_w;
    bufattribute.usHeight = g_PIP_h;   
    bufattribute.usLineOffset = g_PIP_w * g_PIP_bpp;
    bufattribute.colordepth = MMPD_GRAPHICS_COLORDEPTH_16;
    bufattribute.ulBaseAddr = PIP_FB0;
    MMPD_Graphics_SetBufAttributes(0, &bufattribute);

    
    rect.usLeft = 0;
    rect.usTop = 0;
    rect.usWidth = g_PIP_w/2;
    rect.usHeight = g_PIP_h/2;
    MMPD_Graphics_SetKeyColor(MMPD_GRAPHICS_FG_COLOR, 0xF800);
    MMPD_Graphics_DrawRectToBuf(0, &rect, MMPD_GRAPHICS_SOLID_FILL, GRA_ROP_SRCCOPY);

    rect.usLeft = g_PIP_w/2;
    rect.usTop = 0;
    rect.usWidth = g_PIP_w/2;
    rect.usHeight = g_PIP_h/2;
    MMPD_Graphics_SetKeyColor(MMPD_GRAPHICS_FG_COLOR, 0x07E0);
    MMPD_Graphics_DrawRectToBuf(0, &rect, MMPD_GRAPHICS_SOLID_FILL, GRA_ROP_SRCCOPY);

    rect.usLeft = 0;
    rect.usTop = g_PIP_h/2;
    rect.usWidth = g_PIP_w/2;
    rect.usHeight = g_PIP_h/2;
    MMPD_Graphics_SetKeyColor(MMPD_GRAPHICS_FG_COLOR, 0x001F);
    MMPD_Graphics_DrawRectToBuf(0, &rect, MMPD_GRAPHICS_SOLID_FILL, GRA_ROP_SRCCOPY);
    ...
    rect.usWidth = g_PIP_w/2;
    rect.usHeight = g_PIP_h/2;
    MMPD_Graphics_SetKeyColor(MMPD_GRAPHICS_FG_COLOR, 0xFFFF);
    MMPD_Graphics_DrawRectToBuf(0, &rect, MMPD_GRAPHICS_SOLID_FILL, GRA_ROP_SRCCOPY);

    // Draw first line with yellow color
    rect.usLeft = 0;
    rect.usTop = 0;
    rect.usWidth = g_PIP_w/2;
    rect.usHeight = 1;
    MMPD_Graphics_SetKeyColor(MMPD_GRAPHICS_FG_COLOR, 0xFFE0);
    MMPD_Graphics_DrawRectToBuf(0, &rect, MMPD_GRAPHICS_SOLID_FILL, GRA_ROP_SRCCOPY);

    return;
}
#else // Using memory write to fill RGB pattern
{
    int     x, y;
...
#endif // End of RTNA_LCD_RGB_Test()
