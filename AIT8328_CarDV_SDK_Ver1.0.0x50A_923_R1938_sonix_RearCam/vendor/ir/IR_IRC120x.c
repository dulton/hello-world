...
//	RTNA_DBG_PrintByte(0, KeyCode[0]);
//	RTNA_DBG_PrintByte(0, KeyCode[1]);
//	RTNA_DBG_PrintByte(0, KeyCode[2]);
//	RTNA_DBG_PrintByte(0, KeyCode[3]);
...
static MMP_ERR IRC120x_IR_Initial(struct _3RD_PARTY_IR *pthis, IR_ATTRIBUTE *pgsnr_attr)//R_DATA *ir_data)
{
    #ifdef IR_INTR
    if (IR_INTR != MMP_GPIO_MAX) {
        AHC_ConfigGPIOPad(IR_INTR, PAD_NORMAL_TRIG | PAD_PULL_UP);
        AHC_ConfigGPIO(IR_INTR, MMP_FALSE);
    }
    #endif
