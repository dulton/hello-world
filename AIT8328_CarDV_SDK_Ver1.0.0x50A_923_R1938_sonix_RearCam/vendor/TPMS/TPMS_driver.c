...
void TPMS_InitPathUART(void)
{
	...
	MMPF_Uart_EnableRx(TPMS_UART_NUM, 16,(UartCallBackFunc *)&TPMS_String_RxCallback);
...
{
	pTPMSPack			= (TPMSPACK*)(m_ulTPMSBuffStartAddr);
	pTPMSInfo 			= (TPMSINFO*)(m_ulTPMSBuffStartAddr + 100);

	memset(pTPMSPack, 0, sizeof(TPMSPACK));
	memset(pTPMSInfo, 0, sizeof(TPMSINFO));
...
void TPMS_PaeserMain(MMP_BYTE *pBuff, MMP_LONG iBuff_sz, TPMSINFO *pInfo)
{
	...
	do
	{
		...
		{
			iError = TPMS_ParsePack(pBuff+iParsed ,iRead ,pTPMSPack);
		    TPMS_Pack2info(pTPMSPack, pInfo);
...
{	
	...
	memset(&m_tempInfo, 0, sizeof(TPMSINFO));
	m_tempInfo.sTire[0].ubWheelPosition = 65535;
...
{	
	MMP_BOOL err = MMP_TRUE;
	
	err &= ((m_tempInfo.sTire[0].ubWheelPosition == 0) ? MMP_TRUE : MMP_FALSE);
	err &= ((m_tempInfo.sTire[1].ubWheelPosition == 1) ? MMP_TRUE : MMP_FALSE);
	err &= ((m_tempInfo.sTire[2].ubWheelPosition == 2) ? MMP_TRUE : MMP_FALSE);
	err &= ((m_tempInfo.sTire[3].ubWheelPosition == 3) ? MMP_TRUE : MMP_FALSE);
	
	if (err)
	{
		if (m_tempInfo.sTire[0].ubWheelDataUpdate == 1 &&
			m_tempInfo.sTire[1].ubWheelDataUpdate == 1 &&
			m_tempInfo.sTire[2].ubWheelDataUpdate == 1 &&
			m_tempInfo.sTire[3].ubWheelDataUpdate == 1 )
		{	
			err = MMP_TRUE;
		}
		else
...
{ 	
	...
	if (m_curMode == TPMS_MODE_LEARN)
	{
		...
		if (STRCMP((INT8*)"$LEARN#", (INT8*)szTPMSPack) == 0)
		{
			m_bLearnReturn = MMP_TRUE;
			TPMS_SetCommand(TPMSCMD_EXIT_LEARN_ID_MODE);
			AHC_OS_GetTime(&timeRelearnStart);
		}
		
		if (m_bLearnReturn)
		{
			TPMS_PaeserMain(szTPMSPack, uiTPMSPackLength, &m_tempInfo);
			if (TPMS_bReceiveAll())
			{
				printc("Learn finish\n");
				memcpy(pTPMSInfo, &m_tempInfo, sizeof(TPMSINFO));
			...
			{
				AHC_OS_GetTime(&timeRelearnEnd);
				if (timeRelearnEnd-timeRelearnStart >= 3000)
				{
					printc("Learn re-start\n");					
					memcpy(pTPMSInfo, &m_tempInfo, sizeof(TPMSINFO));
		...
		TPMS_PaeserMain(szTPMSPack, uiTPMSPackLength, pTPMSInfo);
}
