...
void GPS_ParserInitialize_NMEA0183(void)
{
	...
	memset(pGPSInfo+sizeof(MMP_HANDLE), 0, sizeof(NMEAINFO));
...
void GPS_SetCommand_NMEA0183(NMEACOMMAND_TYPE type)
{
...
#ifndef CAR_DV
	#if 0
	printc("Send Uart command %d bytes via TX :\r\n", ublength);
	...
	printc("\n");
	#endif
	err = MMPF_Uart_Write(GPS_UART_NUM, chCommand, ublength);
	if (err)
		printc("Send Uart Tx error 0x%x\r\n", err);
#endif
}
...
//  Description :
//------------------------------------------------------------------------------
MMP_ERR GPS_SetCommand(void *CommAry)
{
    char 		chCommand[sizeof(CommAry)];
    MMP_UBYTE 	ublength = 0;
    MMP_ERR 	sRet = MMP_ERR_NONE;

    MEMSET(chCommand, 0, sizeof(CommAry));
    MEMCPY(chCommand, CommAry, sizeof(CommAry));
    ublength = sizeof(CommAry);

    sRet = MMPF_Uart_Write(GPS_UART_NUM, chCommand, ublength);
