...
#define GPS_RAW_NORM_BUF_TIME			10   //sec
#else
...
#if (C005_CRUISE_SPEED_SETTING)
MMP_UBYTE			ubOverCruiseSpeed = 0;
#endif
...
void GPSCtrl_Enable(AHC_BOOL bEnable)
{
	...
	{
        AHC_ConfigGPIOPad(GPS_MODULE_EN_GPIO, PAD_OUT_DRIVING(0));
		AHC_ConfigGPIO(GPS_MODULE_EN_GPIO, AHC_TRUE);
		AHC_SetGPIO(GPS_MODULE_EN_GPIO, bEnable);
	}
...
MMP_BOOL GPSCtrl_LinkGPSTime2RTC(NMEATIME *sGPS)
{
    ...
    #if defined(GPS_TO_RTC_TIME_OFFSET) &&  (GPS_TO_RTC_TIME_OFFSET != 0 )
    {
        ...
        #if(DAYLIGHT_SAVING == 1) && (C005_MENU_DAYLIGHT_SAVING_TIME == 1)
        switch(MenuSettingConfig()->uiDaylightSavingTime)
        {
        	case 0:
        		DateTime += 0;
        	break;
...
#if(C005_CRUISE_SPEED_SETTING)
MMP_ULONG GPSCtrl_GetCruiseSpeedLimit(MMP_UBYTE ubSettings)
{
	MMP_ULONG uiCruiseSpeedLimit = 0;
	switch(ubSettings)
    {
    	case SPEEDCAM_CRUISE_SPEED_70KMH:
    		uiCruiseSpeedLimit = 70;
    	break;
    	
    	case SPEEDCAM_CRUISE_SPEED_80KMH:
    		uiCruiseSpeedLimit = 80;
    	break;
    	
    	case SPEEDCAM_CRUISE_SPEED_90KMH:
    		uiCruiseSpeedLimit = 90;
    	break;
    	
    	case SPEEDCAM_CRUISE_SPEED_100KMH:
    		uiCruiseSpeedLimit = 100;
    	break;
    	
    	case SPEEDCAM_CRUISE_SPEED_110KMH:
    		uiCruiseSpeedLimit = 110;
    	break;
    	
    	case SPEEDCAM_CRUISE_SPEED_120KMH:
    		uiCruiseSpeedLimit = 120;
    	break;
    	
    	case SPEEDCAM_CRUISE_SPEED_130KMH:
    		uiCruiseSpeedLimit = 130;
    	break;
    	
    	case SPEEDCAM_CRUISE_SPEED_140KMH:
    		uiCruiseSpeedLimit = 140;
    	break;
    	
    	default:
    		uiCruiseSpeedLimit = 65535;
    	break;
    }
    return uiCruiseSpeedLimit;
}
MMP_UBYTE GPSCtrl_IsOverCruiseSpeedLimit(MMP_UBYTE ubCruiseSpeedSetting)
{
	MMP_ULONG uiCruiseSpeedLimit;
    ...
    uiCruiseSpeedLimit = GPSCtrl_GetCruiseSpeedLimit(ubCruiseSpeedSetting);
    
    if(ubCruiseSpeedSetting != 0)
    {
    	if( ((MMP_ULONG)pInfo->dwSpeed) > uiCruiseSpeedLimit)
   		{
   			return MMP_TRUE;
   		}
   	}
...
#endif//#if(C005_CRUISE_SPEED_SETTING)
...
AHC_BOOL AHC_RTC_NeedToUpdate(void) //Only for GPS.  Don't put it into common code.
{
    ...
    return AHC_TRUE;
    #else
    if(uwYear <= RTC_DEFAULT_YEAR || MenuSettingConfig()->uiGPSTimeSync)
        return AHC_TRUE;
    else
        return AHC_FALSE;
    #endif
	        ...
	        if(ubStartRecordWithGPSInfo)
        	{
		    	...
		    	else
		    	{
					...
					if(ulGPSCount_0 == GPS_CONFIG_MAXGPSINFONUM)
					{
						...
						printc("Buf0-ulGPSInfoFileByte %d\r\n",ulGPSInfoFileByte);
					}
					#endif		
					#if (GPS_KML_FILE_ENABLE == 1)
...
{	
	...
	if(uiGetCurrentState() == UI_VIDEO_STATE && AHC_IsSDInserted() && AHC_GetMountStorageMediaStatus() && AHC_SDMMC_GetMountState() )
	#elif(TWOSD_WORK_MODEL == TWOSD_WORK_MODEL_COMPLEMENTARY)
	if(uiGetCurrentState() == UI_VIDEO_STATE && (AHC_IsSDInserted() || AHC_Is2ndSDInserted()) && AHC_GetMountStorageMediaStatus() && AHC_SDMMC_GetMountState() )
	#endif
...
{	
	...
	MMP_UBYTE ubIndex = GPSCtrl_GetGPSRawIndex_Normal();	
	if(AHC_IsEmerRecordStarted() && AHC_IsSDInserted())
	{
		...
		if(uiGetParkingModeEnable() && AHC_IsSDInserted())
		#else
		if (AHC_IsSDInserted())
		#endif
		{
			//printc("Gsensor write buffer\r\n");
			if((ulGPSRawNormBufLen[ubIndex] + usLength) < GPS_RAW_NORM_BUF_1SEC)
			{
				memcpy(&ubGPSRawNormBuf[ubIndex][ulGPSRawNormBufLen[ubIndex]],pBuff,usLength);
				ulGPSRawNormBufLen[ubIndex] += usLength;
...
void GPSCtrl_IncGPSRawIndex_Normal(void)
{
	...
	ubGPSRawNormBufIndex = (ubGPSRawNormBufIndex)%(10);
	#else
...
#endif //#if (EMERGENCY_RECODE_FLOW_TYPE == EMERGENCY_RECODE_SWITCH_FILE || C005_PARKING_CLIP_Length_10PRE_10REC == 1)
