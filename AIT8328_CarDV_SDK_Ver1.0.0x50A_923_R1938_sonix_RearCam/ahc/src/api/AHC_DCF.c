...
static DCF_REGION_ID    m_sDCFRegionID	= DCF_REGION_ID_4;
/**
@brief the case(DCF_FILE_DELETE) of AHC_DCF_FileOperation().
...
#if defined(CAR_DV)//Hidden the system directory
	{
		AHC_FS_ATTRIBUTE	sFileAttribute; 
	    AHC_FS_FILETIME		sFileTime;
		UINT32				uiFileSize;
...
#if defined(CAR_DV) && (LIMIT_MAX_LOCK_FILE_NUM)
		{
			extern UINT32 m_ulLockFileNum;
...
AHC_BOOL AHC_DCF_MoveFile(DCF_DB_TYPE sFromType, DCF_DB_TYPE sToType, UINT16 uwDirKey, INT8 *pchFileName, AHC_BOOL bReadOnly)
{
    ...
    */
    ...
    {
        AHC_DCF_FileOperation((UINT8*)chDestName,(UINT8*)chDestFileName, DCF_FILE_READ_ONLY, NULL, NULL);
    }else{
        AHC_DCF_FileOperation((UINT8*)chDestName,(UINT8*)chDestFileName, DCF_FILE_NON_READ_ONLY, NULL, NULL);
    }
        ...
        if((i == MAX_LOCK_FILE_NUM-1)&&(m_ulLockFileTable[i] != 0xFFFFFFFF))
        {
            ...
            if(bUpdate == AHC_TRUE)
            {
                ...
                else
                {
                    ...
                    ahc_ret = AHC_DCF_FileOperation((UINT8*)chPathName, (UINT8*)chRenameFileName, DCF_FILE_DELETE, NULL, NULL);
                    if(ahc_ret == AHC_FALSE)
                    {
