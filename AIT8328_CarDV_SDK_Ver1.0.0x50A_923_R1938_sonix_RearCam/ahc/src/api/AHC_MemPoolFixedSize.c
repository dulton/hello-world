...
{
#if UCOS2
    if (AHC_OS_AcquireSem(hPool->smLock, uiTimeOutTick) != OS_NO_ERR) 
    {
        return AHC_FALSE;
    }
#endif // UCOS2
...
#if UCOS2
	// Wait forever
    if (AHC_OS_AcquireSem(hPool->smLock, 0) != OS_NO_ERR) 
    {
        return AHC_FALSE;
    }
#endif // UCOS2
...
{
#if UCOS2
    if (AHC_OS_ReleaseSem(hPool->smLock) != OS_NO_ERR) 
    {
        return AHC_FALSE;
    }
#endif // UCOS2
...
{    
    ...
    {
    	#if UCOS2
    	AHC_OS_SEMID TempsmLock;
		#endif 

		TempsmLock = hPool->smLock;
		
        AHC_MemPoolFS_Lock(hPool);
        MEMSET( hPool, 0, sizeof(SMEMPOOLFIXEDSIZE) );
        AHC_MemPoolFS_Unlock(hPool);

	    if( AHC_OS_DeleteSem(TempsmLock) == 0xFF )
	    {
	        return AHC_FALSE;
	    }	
        //AHC_MemPoolFS_LockUninit(hPool);
        return AHC_TRUE;
    }
    else
    {
