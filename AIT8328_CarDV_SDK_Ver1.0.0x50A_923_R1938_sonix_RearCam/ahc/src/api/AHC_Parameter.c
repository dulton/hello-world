...
extern AHC_MEMORY_LOCATION glAhcMemoryLocation[AHC_MEMORY_LOCATION_MAX];
...
AHC_BOOL AIHC_PARAM_Initialize(void) {
	...
	//glAhcParameter.TimerID 			= 0x0;	//Set the specific timer for AHC_SetTimer().
...
#ifdef CAR_DV		// TY Miao - TBD

const char txtSetFile[] = "SF:1:\\useset.txt";
...
{	
	...
	if (accesspath & AHC_DEFAULT_USER_FAT) {
		ExportMenuAtom((char*)txtSetFile, TXT_NO_COMMENT | TXT_MENU_ALL);
	}
...
Label_FileProc :
	...
	err = AHC_FS_FileRead(ulFileId, readback,sizeof(MenuInfo),&ulActualSize);
...
//------------------------------------------------------------------------------
//  Function    : AHC_PARAM_Menu_Write
//  Description : Write default value to storage device
//------------------------------------------------------------------------------
AHC_BOOL AHC_PARAM_Menu_Write(AHC_VALUE_ACCESS_PATH accesspath)
{	
	char*   path="SM:3:\\MenuSetting.bin"; 
	...
	STRCPY(path,"SF:0:\\");
	STRCAT(path,AHC_MENU_FILENAME);
	DramToFile(path, (INT8*)MenuSettingConfig(),sizeof(MenuInfo));	
	#if (!LCD_ROTATE_WITH_HALL_SENSOR && PANEL_OSD_FILP_ENABLE)
	{ //Write LCD status to SIF before power off
	FS_SFLASH_Driver.pfInitDevice(0);
	FS_SFLASH_Driver.pfInitMedium(0);
	FS_SFLASH_Driver.pfReadBurst(0,767,1,(UINT8 *)(0x4000000));
	memcpy((UINT8 *)0x4000200,(UINT8 *)0x4000000,512);
	*(UINT8 *)(0x4000000+511) = (MenuSettingConfig()->uiLCDRotate)?(0):(1); //0 ->Normal 1->LCD flip;
	FS_SFLASH_Driver.pfWriteBurst(0, 767, 1,(INT8*)(0x4000000));
	FS_SFLASH_Driver.pfReadBurst(0,767,1,(UINT8 *)(0x4000200));
	}
	#endif
	goto Label_End;	
	#else
	...
	{
		case AHC_DEFAULT_FACTORY_FAT:
			DramToFile(path, (INT8*)FactoryMenuSettingConfig(),sizeof(MenuInfo));
		break;
		case AHC_DEFAULT_USER_FAT:
			MenuSettingConfig()->uiBatteryVoltage = BATTERY_VOLTAGE_FULL;
			DramToFile(path, (INT8*)MenuSettingConfig(),sizeof(MenuInfo));
