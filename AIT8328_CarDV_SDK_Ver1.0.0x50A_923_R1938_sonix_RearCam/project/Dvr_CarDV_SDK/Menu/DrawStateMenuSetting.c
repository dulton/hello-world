...
void OSDDraw_CurrentRTCTime(UINT16 ubID, UINT32 x, UINT32 y, AHC_BOOL bLarge, UINT8 ubMag, AHC_BOOL bFlag, GUI_COLOR bkColor, INT32 Para1)
{
    ...
    {	
        ...
        AHC_OSDSetBkColor(ubID, bkColor);
		#else
        AHC_OSDSetColor(OSD_COLOR_YELLOW);
...
void OSDDraw_DigitZoomBar(UINT16 ubID, UINT32 x, UINT32 y, AHC_BOOL bLarge, UINT8 ubMag, AHC_BOOL bFlag, GUI_COLOR bkColor, INT32 Para1, UINT8 ubMode)
{
    ...
    AHC_OSDSetColor(ubID, OSD_COLOR_WHITE);
    AHC_OSDSetBkColor(ubID, bkColor);
    AHC_OSDSetFont(ubID, &Font1);
	#else
    AHC_OSDSetColor(OSD_COLOR_WHITE);
    AHC_OSDSetBkColor(bkColor);
    AHC_OSDSetFont(&Font1);
	#endif
	...
	//Draw Zoom Ratio
	#ifdef ENABLE_GUI_SUPPORT_MULTITASK
	AHC_OSDSetFont(ubID, &Font2);
	#else
	AHC_OSDSetFont(&Font2);
	#endif
...
void OSDDrawSetting_MediaType(UINT16 ubID, UINT32 x, UINT32 y, AHC_BOOL bLarge, UINT8 ubMag, AHC_BOOL bFlag, GUI_COLOR bkColor, INT32 Para1)
{
    ...
    else
    {
        ...
        else if(AHC_FALSE == AHC_SDMMC_GetMountState())
        {
        	IconID = BMICON_MEDIA_NO_SD;
        }
...
#if (MENU_MANUAL_FACE_DETECT_EN && OSD_SHOW_FACE_DETECTION)
void OSDDrawSetting_FaceDetect(UINT16 ubID, UINT32 x, UINT32 y, AHC_BOOL bLarge, UINT8 ubMag, AHC_BOOL bFlag, GUI_COLOR bkColor, INT32 Para1)
{
    GUI_BITMAP 	IconID = bmIcon_FD_OFF;
    UINT8 		OffsetX = 7;
    ...
    if( AHC_FALSE == bFlag )
    {
        ...
        if(bLarge==LARGE_SIZE)
        	...
        	AHC_OSDDrawFillRect(ubID, x-OffsetX, y,   x+IconID.XSize,    y+IconID.YSize);
    ...
    {
        switch ( MenuSettingConfig()->uiFaceTouch )
        {
            ...
            case FACE_DETECT_TOUCHSPOT:
                 IconID = bmIcon_FaceTouch_TouchSpot;
                 OffsetX = 7;
            break;
        #endif
        #if (MENU_MANUAL_FACE_DETECT_OFF_EN)    
            case FACE_DETECT_OFF:
                 IconID = bmIcon_FD_OFF;
                 OffsetX = 0;
            break;
        #endif    
            default:
...
#if (MENU_MANUAL_SMILE_DETECT_EN && OSD_SHOW_SMILE_DETECTION)
...
{
    GUI_BITMAP 	IconID = bmIcon_Smile_Detection_Off;

    if (AHC_FALSE == AIHC_GetOSDInitStatus()) { AHC_PRINT_RET_ERROR(gbAhcDbgBrk,0); return; }

    BEGIN_LAYER(ubID);

    if( AHC_FALSE == bFlag || SMILE_DETECT_OFF == MenuSettingConfig()->uiSmileDetect )
    {
 		#ifdef ENABLE_GUI_SUPPORT_MULTITASK
        AHC_OSDSetColor(ubID, bkColor);
		AHC_OSDSetBkColor(ubID, bkColor);
    ...
    {
        switch ( MenuSettingConfig()->uiSmileDetect )
        {
        #if (MENU_MANUAL_SMILE_DETECT_OFF_EN)
            case SMILE_DETECT_OFF    :
                 IconID = bmIcon_Smile_Detection_Off;
            break;
        #endif
        #if (MENU_MANUAL_SMILE_DETECT_1PEOPLE_EN)    
            case SMILE_DETECT_1PEOPLE:
                 IconID = bmIcon_Smile_Detection_1People;
            break;
        #endif
        #if (MENU_MANUAL_SMILE_DETECT_2PEOPLE_EN)    
            case SMILE_DETECT_2PEOPLE:
                 IconID = bmIcon_Smile_Detection_2People;
            break; 
        #endif    
            default:
                 IconID = bmIcon_Smile_Detection_Off;
...
void OSDDrawSetting_BatteryDummy(UINT16 ubID, UINT32 x, UINT32 y, AHC_BOOL bLarge, UINT8 ubMag)
{
	...
	if (AHC_Get_WiFi_Streaming_Status()) {
		...
		#else
		AHC_OSDSetColor(CFG_DRAW_WIFI_STREAMING_MONO_PATTERN);
		AHC_OSDSetBkColor(CFG_DRAW_WIFI_STREAMING_MONO_PATTERN);
	    #endif
	}
#endif
...
{    
    ...
    if(AHC_FALSE == AHC_GetChargerStatus()) {
...
void OSD_Draw_TestMessage(UINT32 Left, UINT32 Top, char result,char sText[20])
{
...
#else
    AHC_OSDSetColor(OSD_COLOR_BLUE );
    AHC_OSDSetBkColor(OSD_COLOR_BLACK );
    AHC_OSDSetFont(&GUI_Font13_1 );
#endif
	...
	if(result==0)//test OK
	{
		...
		#else
		AHC_OSDSetBkColor(OSD_COLOR_GREEN );
	    #endif
		AHC_OSDDrawTextInRectEx(	bID0, 
									(UINT8*)sOK, 
	...
	else if(result==1)//test NG
	{
		...
		#else
		AHC_OSDSetBkColor(OSD_COLOR_RED);
	    #endif
		AHC_OSDDrawTextInRectEx(	bID0, 
									(UINT8*)sNG, 
									CaptureRECT.uiLeft+80, 
	...
	else if(result==2)//default
	{
		...
		#else
		AHC_OSDSetBkColor( OSD_COLOR_YELLOW );
	    #endif
		AHC_OSDDrawTextInRectEx(	bID0, 
									(UINT8*)sUnDo, 
									CaptureRECT.uiLeft+80, 
	...
	else if(result==3)//show version
	{
		...
		#else
		AHC_OSDSetBkColor( OSD_COLOR_GRAY );
	    #endif
		AHC_OSDDrawTextInRectEx(	bID0, 
									(UINT8*)sVer, 
									CaptureRECT.uiLeft+80, 
