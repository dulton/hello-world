...
void MenuDraw_UIMode(UINT16 uwDispID)
{
    ...
    {
        UIDrawHdmi_VideoFuncEx(uwDispID, HDMI_GUI_VIDEO_UI_MODE, AHC_FALSE, AHC_TRUE, OSD_COLOR_TITLE, NULL_PARAM);
    }
    else if(TVFunc_IsInTVMode())
    {
        UIDrawTV_VideoFuncEx(uwDispID, TV_GUI_VIDEO_UI_MODE, AHC_TRUE, AHC_TRUE, OSD_COLOR_TITLE, NULL_PARAM);
    }
    else
    {
        UIDrawVideoFuncEx(uwDispID, VIDEO_GUI_UI_MODE, AHC_TRUE, AHC_TRUE, OSD_COLOR_TITLE, NULL_PARAM);
    }   
}
...
void MenuDraw_ClearMediaType(UINT16 uwDispID)
{
    ...
    {
        UIDrawHdmi_VideoFuncEx(uwDispID, HDMI_GUI_VIDEO_SD_STATE, AHC_TRUE, AHC_TRUE, OSD_COLOR_TITLE, NULL_PARAM);
    }
    else if(TVFunc_IsInMenu())
    {
        UIDrawTV_VideoFuncEx(uwDispID, TV_GUI_VIDEO_SD_STATE, AHC_TRUE, AHC_TRUE, OSD_COLOR_TITLE, NULL_PARAM);
    }
    else
    {
        UIDrawVideoFuncEx(uwDispID, VIDEO_GUI_SD_STATE, AHC_TRUE, AHC_TRUE, OSD_COLOR_TITLE, NULL_PARAM);
    }   
}
...
void MenuDraw_MediaType(UINT16 uwDispID)
{
    if(HDMIFunc_IsInHdmiMode())
    {
        UIDrawHdmi_VideoFuncEx(uwDispID, HDMI_GUI_VIDEO_SD_STATE, AHC_FALSE, AHC_TRUE, OSD_COLOR_TITLE, NULL_PARAM);
    }
    else if(TVFunc_IsInTVMode())
    {
        UIDrawTV_VideoFuncEx(uwDispID, TV_GUI_VIDEO_SD_STATE, AHC_TRUE, AHC_TRUE, OSD_COLOR_TITLE, NULL_PARAM);
...
void MenuDraw_BatteryStatus(UINT16 uwDispID)
{
    ...
    if(HDMIFunc_IsInHdmiMode())
    {
        UIDrawHdmi_VideoFuncEx(uwDispID, HDMI_GUI_VIDEO_BATTERY, AHC_TRUE, AHC_TRUE, OSD_COLOR_TITLE, NULL_PARAM);
    }
    else if(TVFunc_IsInTVMode())
    {
        UIDrawTV_VideoFuncEx(uwDispID, TV_GUI_VIDEO_BATTERY, AHC_TRUE, AHC_TRUE, OSD_COLOR_TITLE, NULL_PARAM);
    }
    else
    {
        UIDrawVideoFuncEx(uwDispID, VIDEO_GUI_BATTERY, AHC_TRUE, AHC_TRUE, OSD_COLOR_TITLE, NULL_PARAM);
    }
#endif
}
    ...
    switch(TVFunc_Status())
    {
        ...
        case AHC_TV_MOVIE_PB_STATUS:
            if(!m_DelFile)
            {
                ...
                DrawTVMoviePB_Status(uwDispID, POS_TV_MOV_STATE_X, POS_TV_MOV_STATE_Y, LARGE_SIZE, MAG_2X, AHC_TRUE, OSD_COLOR_TRANSPARENT, NULL_PARAM);
            }
            else
...
void MenuMainDrawBackCurtain(UINT16 uwDispID, GUI_COLOR bkColor)
{
    ...
    #else
    if(TVFunc_IsInTVMode() || HDMIFunc_IsInHdmiMode())
        AHC_OSDSetColor(TV_MENU_BK_COLOR);
    else
...
void DrawSubItem(UINT16 uwDispID, int iItem, int iTotalItems, UINT8 uLang, UINT32 iStrID, const GUI_BITMAP* IconID, UINT32 barId, GUI_COLOR clrFont, GUI_COLOR clrBack, GUI_COLOR clrSelect)
{
    ...
    if(AHC_TRUE == AHC_GetChargerStatus())
    {
        ...
        if(TVFunc_IsInTVMode() || HDMIFunc_IsInHdmiMode())
            ...
            #else
            AHC_OSDSetBkColor(OSD_COLOR_TV_BACKGROUND);
            #endif
        else
            #ifdef ENABLE_GUI_SUPPORT_MULTITASK
            AHC_OSDSetBkColor(uwDispID, MENU_BACKGROUND_COLOR);
            #else
            AHC_OSDSetBkColor(MENU_BACKGROUND_COLOR);
            #endif
    }
    ...
    if(0/*TVFunc_IsInTVMode() || HDMIFunc_IsInHdmiMode()*/)
    {
        tmpRECT.uiLeft   = rc.uiLeft            + STR_RECT_OFFSET_X;
...
void MenuDrawSubMenuDescription(UINT16 uwDispID, UINT32 iStrID)
{
    ...
    bkColor = (TVFunc_IsInTVMode() || HDMIFunc_IsInHdmiMode())?(TV_MENU_BK_COLOR):(OSD_COLOR_TITLE);
...
#if (UI_MODE_SELECT_EN) || (USB_MODE_SELECT_EN) || (BROWSER_MODE_SELECT_EN)
