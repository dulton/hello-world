...
void MenuStatePlaybackMode(UINT32 ulEvent, UINT32 ulPosition)
{
    ...
    if( ulResult == MENU_ERR_EXIT )
    {
        ...
        if( AHC_CheckCurSysMode(AHC_MODE_PLAYBACK) || AHC_CheckCurSysMode(AHC_MODE_IDLE) )
		{
			if(Deleting_InBrowser || Protecting_InBrowser)
			{
				printc("Force Switch Browser!\r\n");
				
				if(uiGetLastState()==UI_SLIDESHOW_STATE)
				{
					if(ssFileType==PLAYBACK_IMAGE_TYPE)
						SetCurrentOpMode(JPGPB_MODE);
					else if(ssFileType==PLAYBACK_VIDEO_TYPE)
