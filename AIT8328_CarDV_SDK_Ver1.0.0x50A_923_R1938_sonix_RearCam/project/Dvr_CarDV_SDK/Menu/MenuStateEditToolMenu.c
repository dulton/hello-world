...
L_out:
		...
		{		
			...
			{			
				if(ssFileType!=PLAYBACK_TYPE_UNSUPPORTED)
				{
					SlideShowTimer_Stop();
					SlideShowFunc_Stop();
				}
		...
		if(AHC_CheckCurSysMode(AHC_MODE_PLAYBACK))
		{
			...
			else
			{
		    	...
		    	if(TVFunc_IsInTVMode())
		    		TVFunc_PlaybackEditOneFile(DCF_FILE_DELETE);
		    	else if(HDMIFunc_IsInHdmiMode())
		    		HDMIFunc_PlaybackEditOneFile(DCF_FILE_DELETE);	  
			}
		}
...
AHC_BOOL MenuItemDeleteAll(PSMENUITEM pItem, AHC_BOOL bIsHover)
{
	...
	{  
		...
		if(!AHC_UF_FileOperation_ByIdx(CurObjIdx, DCF_FILE_DELETE, NULL, NULL))
		{
			AHC_WMSG_Draw(AHC_TRUE, WMSG_CANNOT_DELETE, 3);
			CurObjIdx++;
	...
	#if (TVOUT_ENABLE)
	else if(AHC_TV_BROWSER_STATUS == TVFunc_Status() && TVFunc_IsInTVMode()) 
    {
	    if( JPGPB_MODE == GetCurrentOpMode())
	    {
	        AHC_UF_SetFreeChar(0, DCF_SET_ALLOWED, (UINT8 *) SEARCH_PHOTO);        
...
AHC_BOOL MenuItemProtectOne(PSMENUITEM pItem, AHC_BOOL bHover)
{
		...
		)
	{
		if(AHC_CheckCurSysMode(AHC_MODE_PLAYBACK))
		{
			...
			ret = QuickFileOperation(DCF_FILE_READ_ONLY);
...
AHC_BOOL MenuItemProtectAll(PSMENUITEM pItem, AHC_BOOL bIsHover)
{
	...
	for(i=0;i<MaxDcfObj;i++) 
    {   
		if(!AHC_UF_FileOperation_ByIdx(i, DCF_FILE_READ_ONLY, NULL, NULL))
			return AHC_FALSE;
...
AHC_BOOL MenuItemUnProtectOne(PSMENUITEM pItem, AHC_BOOL bHover)
{
	   ...
	   )
	{
		if(AHC_CheckCurSysMode(AHC_MODE_PLAYBACK))
		{
			...
			ret = QuickFileOperation(DCF_FILE_NON_READ_ONLY);
...
AHC_BOOL MenuItemUnProtectAll(PSMENUITEM pItem, AHC_BOOL bIsHover)
{
	...
	for(i=0;i<MaxDcfObj;i++) 
	{      
		if(!AHC_UF_FileOperation_ByIdx(i, DCF_FILE_NON_READ_ONLY, NULL, NULL))
			return AHC_FALSE;

		if(!AHC_UF_FileOperation_ByIdx(i, DCF_FILE_CHAR_UNLOCK, NULL, NULL))
			return AHC_FALSE;
